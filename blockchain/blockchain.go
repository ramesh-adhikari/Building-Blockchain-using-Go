...
func (bc *Blockchain) SetNeighbors() {
	bc.neighbors = utility.FindNeighbors("127.0.0.1", bc.port, NEIGHBOR_IP_RANGE_START, NEIGHBOR_IP_RANGE_END, BLOCKCHAIN_PORT_RANGE_START, BLOCKCHAIN_PORT_RANGE_END)
...
func (bc *Blockchain) CreateBlock(nonce int, previousHash [32]byte) *Block {
	...
	for _, n := range bc.neighbors {
		endpont := fmt.Sprintf("http://%s/transactions", n)
		client := &http.Client{}
		req, _ := http.NewRequest("DELETE", endpont, nil)
...
func (bc *Blockchain) AddTransaction(sender string, recipent string, value float32, senderPublicKey *ecdsa.PublicKey, s *utility.Signature) bool {
	...
	if bc.veryfyTransactionSignature(senderPublicKey, s, t) {
		...
		// 	log.Println("ERROR: Not enough balence in a wallet")
		// 	return false
...
func (bc *Blockchain) ResolveConflicts() bool {
	...
	for _, n := range bc.neighbors {
		...
		if resp.StatusCode == 200 {
			...
			log.Printf("Error status occur while requesting for chain")
		}
	}
	if longestChain != nil {
	...
	log.Printf("Resolved conflicts not replaced")
	return false
}
...
func (tr *TransactionRequest) Validate() bool {
	if tr.SenderBlockchainAddress == nil || tr.RecipientBlockchainAddress == nil || tr.SenderPublicKey == nil ||
